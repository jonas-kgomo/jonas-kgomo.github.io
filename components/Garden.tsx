...
const Timeline = ({ selectedJob, selectedIndex }) => {
  ...
  return (
    <div className="timeline bg-primary-200 dark:bg-primary-600 py-6 w-1/3 h-76 flex flex-col ">
      {jobs.map((job, index) => (
        <motion.div
          key={index}
          className={` ${selectedIndex ? 'font-base' : 'font-medium'} timeline-entry bg-primary-200 dark:bg-primary-600   px-4   cursor-pointer hover:bg-gray-300"`}
          onClick={() => handleJobClick(index)}
          animate={{ opacity: selectedIndex === index ? '100%' : '50%', 
...
const Sidebar = ({ selectedJobIndex })  => {
  ...
  return (
    <div className="sidebar  h-64 w-2/3 bg-primary-900 text-primary-100 font-medium p-4">
      <div className='flex'>{job.title}  <p className='text-medium pl-2 text-primary-400'>@ {job.company}</p>
     </div>
      <code> {job.range} </code>
